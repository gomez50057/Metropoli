...
const Team = ({ teamName, teamSubName, teamMembers, isTecnicoTeam = false }) => {
  ...
  return (
    <div className={teamStyles["team-container"]} ref={teamRef}>
      <h2 className={teamStyles["team-title"]}>{teamName}</h2>
      <h3 className={teamStyles["team-Subtitle"]}>{teamSubName}</h3>
      {teamMembers.map((member, index) => (
        <div key={index} className={teamStyles["team-card"]}>
          <div className={teamStyles["team-card-image"]}>
            <img src={member.image} alt={member.name} />
          </div>
          <div className={teamStyles["team-card-info"]}>
            <h3>{member.name}</h3>
            <p>{member.position}</p>
