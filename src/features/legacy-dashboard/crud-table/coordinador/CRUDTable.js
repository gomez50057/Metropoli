...
import cRUDTableStyles from '../CRUDTable.module.css';
...
const CRUDTable = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      const response = await axios.get(apiUrl('/acuerdos'));
  ...
  const columns = [
    ...
    columnHelper.accessor('descripcion_avance', {
      ...
      Cell: ({ row }) => (
        <Button
          variant="outlined"
          className={cRUDTableStyles["crud-button"]}
    ...
    }),
    {
      ...
      Cell: ({ row }) => {
        ...
        return (
          <Box display="flex" gap={1} className={cRUDTableStyles["Acciones-con"]}>
            <Button variant="outlined" className={cRUDTableStyles["crud-button"]} onClick={() => handleAction('edit', id)}>Editar</Button>
            <Button variant="outlined" className={cRUDTableStyles["crud-button"]} onClick={() => handleAction('update', id)}>Actualizar</Button>
            <Button variant="outlined" className={cRUDTableStyles["crud-button"]} onClick={() => handleAction('history', id)}>Historial</Button>
          </Box>
        );
      },
      enableSorting: false,
