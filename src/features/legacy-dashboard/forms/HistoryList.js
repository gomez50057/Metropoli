...
import historyListStyles from './HistoryList.module.css'; // Asegúrate de crear y utilizar este archivo CSS
...
const HistoryList = ({ projectId, onClose }) => {
  ...
  useEffect(() => {
    const fetchUpdates = async () => {
      try {
        const response = await axios.get(apiUrl(`/actualizaciones/?acuerdo=${encodeURIComponent(projectId)}`));
  ...
  return (
    <div className={historyListStyles["history-list"]}>
      ...
      <button onClick={onClose} className={historyListStyles["close-button"]}>Cerrar</button>
    </div>
  );
};
