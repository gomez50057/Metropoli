...
import { getTituloZona, getPreposicion, getMunAlc, getMapNom } from '@/data/home';
import { renderParagraphsWithStyles } from "@/data/blogData";
...
const InteractiveMap = () => {
  ...
  useEffect(() => {
    const handleZonaChange = () => {
      const newZona = localStorage.getItem('selectedZonaMetropolitana') || 'ZMVM'; // Obtener la nueva zona seleccionada
  ...
  // Helper: fuerza coma como separador de miles (5,555,555)
  const fmtCommaInt = (v) => {
    if (v === null || v === undefined || v === "") return "No disponible";

    // Quita cualquier cosa que no sea dígito o signo (por si viene "5,555,555" o "5 555 555")
  ...
  useEffect(() => {
    ...
    const createPopupContentMetropolitanas = (feature) => {
      ...
      const poblacionMunicipal = fmtCommaInt(POBMUN);
      const poblacionFemenina = fmtCommaInt(POBFEM);
      const poblacionMasculina = fmtCommaInt(POBMAS);
      const poblacionMetropolitana = fmtCommaInt(POB_ESTATA);
    ...
    const createPopupContentZMVM = (feature) => {
      ...
      const poblacionFemenina = fmtCommaInt(POBFEM);
      const poblacionMasculina = fmtCommaInt(POBMAS);
      const poblacionMetropolitana = fmtCommaInt(POBMETRO);
      const superficieMunicipal = Superficie ? `${Superficie.toFixed(3)} km²` : "No disponible";
